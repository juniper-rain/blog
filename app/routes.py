...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Congratulations, you are now a registered user!Woohoo!')
        return redirect(url_for('login'))
...
def explore():
    ...
    prev_url = url_for('explore', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('index.html', title='Explore', posts=posts.items, next_url=next_url, pre_url=prev_url)
